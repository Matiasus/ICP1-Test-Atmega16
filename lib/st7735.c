...
void HardwareReset(void)
{
  ...
  ST7735_HWRST_PORT |= (1 << ST7735_HWRST_PIN);
  // DDR as output
  ST7735_HWRST_DDR  |= (1 << ST7735_HWRST_PIN); 
  // delay 200 ms
  _delay_ms(200);
  // Reset Low 
  ST7735_HWRST_PORT &= ~(1 << ST7735_HWRST_PIN);
...
{
  // set DDR backlight and data/command as output
  ST7735_DDR |= (1 << ST7735_BL) |
                (1 << ST7735_DC_LD);
  // set high level on Backlight
  ST7735_PORT |= (1 << ST7735_BL);
  // very important hardware reset!!! (avoid sensiting on port where hardware reset is on)
  HardwareReset();
...
uint8_t CommandSend(uint8_t data)
{
  ...
  ST7735_PORT &= ~(1 << ST7735_CS_LD);
  // command (active low)
  ST7735_PORT &= ~(1 << ST7735_DC_LD);
  // transmitting data
  SPDR = data;
